...
def create_spc_alerts(db: Session, target_id: int, patterns: List[Dict[str, Any]], measurement_id: int = None):
    """
    SPC 패턴 감지 결과를 바탕으로 SPC 알림 생성
    """
    for pattern in patterns:
        # Rule ID를 기반으로 SPCRule 찾기 (기본 룰이 없으면 생성)
        rule_name = f"Nelson Rule {pattern['rule']}"
        spc_rule = db.query(models.SPCRule).filter(
            models.SPCRule.name == rule_name
        ).first()
        
        if not spc_rule:
            # 룰이 없으면 생성
            spc_rule = models.SPCRule(
                name=rule_name,
        ...
        # 패턴 발생 위치의 측정 데이터 찾기
        if measurement_id:
            alert_measurement_id = measurement_id
        else:
            # 패턴 위치로부터 해당 측정 데이터 ID 찾기
            measurements = db.query(models.Measurement).filter(
                models.Measurement.target_id == target_id
            ).order_by(models.Measurement.created_at.asc()).all()
            
            pos = pattern.get('position', 0)
            if pos < len(measurements):
                alert_measurement_id = measurements[pos].id
            else:
                continue
        
        # 중복 알림 확인
        existing_alert = db.query(models.SPCAlert).filter(
            models.SPCAlert.measurement_id == alert_measurement_id,
            models.SPCAlert.spc_rule_id == spc_rule.id
        ).first()
        
        if not existing_alert:
            # 새 알림 생성
            new_alert = models.SPCAlert(
                measurement_id=alert_measurement_id,
                spc_rule_id=spc_rule.id,
                status="new",
                description=f"{pattern['description']} at LOT_NO: {pattern.get('lot_no', 'Unknown')}"
            )
            db.add(new_alert)
