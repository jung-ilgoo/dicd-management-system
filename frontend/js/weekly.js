...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let selectedDate = moment().startOf('week'); // 기본값: 이번 주 월요일
    ...
    const initDatePicker = () => {
        ...
        const weekEnd = selectedDate.clone().endOf('week').format('YYYY-MM-DD');
        $('#week-picker').val(`${weekStart} ~ ${weekEnd}`);
    };
    ...
    const loadPreviewData = async () => {
        if (!selectedTargetId || !selectedDate) return;
        ...
        try {
            ...
            // 선택한 날짜(월요일) 기준으로 날짜 문자열 생성
            const dateStr = selectedDate.format('YYYY-MM-DD');
            
            // SPC 분석 데이터 가져오기 (7일 데이터 - 주간 기준)
            const spcData = await api.analyzeSpc(selectedTargetId, 7);
            
            // 통계 데이터 가져오기
            const statsData = await api.getTargetStatistics(selectedTargetId, 7);
    ...
    const resetPreview = () => {
        ...
        document.getElementById('preview-placeholder').innerHTML = '<p class="text-muted">타겟과 날짜를 선택하면 보고서 미리보기가 표시됩니다.</p>';
    ...
    const generateReport = () => {
        ...
        try {
            ...
            // 보고서 생성 API 호출
            api.generateWeeklyReport(selectedTargetId, dateStr);
    ...
    const updateButtonState = () => {
        ...
        generateBtn.disabled = !(selectedTargetId && selectedDate);
    };
